...
% Wallot & Mønster (2018) Calculation of average mutual information (AMI)
% and false-nearest neighbors (FNN) for the estimation of embedding
% parameters of multidimensional time-series in MATLAB Frontiers in
% Psychology, 9: 1679 DOI:10.3389/fpsyg.2018.01679
        ...
        for i=1:nbins
            for j=1:nbins
                if (p_i(i) ~= 0 && p_i(j) ~= 0 && p_ij(i,j) ~= 0)
                    ami(tau+1) = ami(tau+1) + p_ij(i,j) * log2(p_ij(i,j) / (p_i(i) * p_i(j)));
                end
            end
        end
    end
